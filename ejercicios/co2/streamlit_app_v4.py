...
def load_data():
    ...
    if not os.path.exists(shp_path):
        st.error(f"❌ Archivo no encontrado: {shp_path}")
        st.stop()
...
with st.sidebar:
    ...
    # Botón de Reset Global (Truco: Limpia el caché de sesión o recarga)
...
with tab_dashboard:
    ...
    # --- SECCIÓN B: TENDENCIAS Y REGIONES (2 Columnas) ---
    ...
    with col_trend:
        ...
        if sel_paises:
            ...
            fig_trend.update_layout(legend=dict(orientation="h", y=-0.2)) # Leyenda abajo
    ...
    if sel_reg_tree:
        # Filtrar por año límite Y regiones seleccionadas
        ...
        df_tree = df_tree[df_tree['co2'] > 0] # Quitar ceros
        ...
        # Mostrar etiqueta y porcentaje del total
